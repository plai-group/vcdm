...
def calculate_inception_stats(
    ...
    num_workers=11, prefetch_factor=2, device=torch.device('cuda'),
):
    # Rank 0 goes first.
    if dist.get_rank() != 0:
    ...
    detector_url = 'fid-refs/inception-2015-12-05.pkl'
...
def calculate_fid_from_inception_stats(mu, sigma, mu_ref, sigma_ref):
    ...
    print('mu_ref', mu_ref.mean())
    print('sigma_ref', sigma_ref.mean())
    m = np.square(mu - mu_ref).sum()
    s, _ = scipy.linalg.sqrtm(np.dot(sigma, sigma_ref), disp=False)
    fid = m + np.trace(sigma + sigma_ref - s * 2)
    if np.isnan(fid) or (fid < 0.) or (fid > 1e5):
        print(f'Got {fid}. Trying again with inflated sigmas.')
        eps = np.eye(len(sigma)) * 1e-8
        return calculate_fid_from_inception_stats(mu, sigma+eps, mu_ref, sigma_ref+eps)
